...
export const AdminWorkspaceBarChart = ({ workspaces = [] }) => {
    const data = workspaces.map(ws => ({
        name: ws.name.length > 12 ? ws.name.slice(0, 12) + '…' : ws.name,
        Members: ws.membersCount || 0,
        Projects: ws.projectsCount || 0,
    }));

    return (
        <div className="chart-card">
            <h3 className="chart-title">Workspace Activity</h3>
            <div className="chart-body">
                <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                    <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        <XAxis
                            ...
                            axisLine={false}
                            tickLine={false}
                            tick={{ fill: '#64748b', fontSize: 11, fontFamily: 'Inter, sans-serif', fontWeight: 500 }}
                        />
                        <YAxis
                            axisLine={false}
                            tickLine={false}
                        ...
                        />
                        <Tooltip cursor={{ fill: 'rgba(0,0,0,0.02)' }} content={<CustomTooltip />} />
                        <Bar dataKey="Members" fill="#3b82f6" radius={[6, 6, 0, 0]} barSize={24} />
                        <Bar dataKey="Projects" fill="#10b981" radius={[6, 6, 0, 0]} barSize={24} />
                    </BarChart>
                </ResponsiveContainer>
            </div>
            <div className="chart-legend">
                <div className="chart-legend-item">
                    <div className="chart-legend-color" style={{ backgroundColor: '#3b82f6' }} />
                    <span>Members</span>
                </div>
                <div className="chart-legend-item">
                    <div className="chart-legend-color" style={{ backgroundColor: '#10b981' }} />
                    <span>Projects</span>
                </div>
            </div>
        </div>
...
export const AdminStatsDonut = ({ totalUsers = 0, totalWorkspaces = 0, totalProjects = 0 }) => {
    const data = [
        { name: 'Users',      value: totalUsers,      color: '#3b82f6' },
        { name: 'Workspaces', value: totalWorkspaces, color: '#8b5cf6' },
